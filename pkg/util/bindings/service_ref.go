...
func (k ServiceRefBindingProvider) Translate(ctx BindingContext, endpointCtx EndpointContext, e v1.Endpoint) (*Binding, error) {
	...
	svc, err := kubernetes.LookupService(ctx.Ctx, ctx.Client, e.Ref.Namespace, e.Ref.Name)
	if err != nil {
		return nil, err
	}
	if svc == nil {
		return nil, fmt.Errorf("could not load a Service with name %s in namespace %s", e.Ref.Name, e.Ref.Namespace)
	}
