...
func (action *monitorAction) Handle(ctx context.Context, integration *v1.Integration) (*v1.Integration, error) {
	if isInImportRunning(integration) {
		// The first time we import from Deployment, we don't monitor for any change as there could be
		// differences between the resources generated by the operator and the ones provided by the user.
		// These conflicts are generated by K8S client caching mechanism and
		// will eventually disappear as the resources managed by the operator will take over
		// the resources managed by the user.
		integration.Status.SetCondition(
			v1.IntegrationConditionImported,
			corev1.ConditionTrue,
			v1.IntegrationConditionImportReason,
			fmt.Sprintf("Imported from deployment %s", integration.Annotations["camel.apache.org/imported-by"]),
		)
		integration.Status.Phase = v1.IntegrationPhaseRunning
...
func isInImportRunning(it *v1.Integration) bool {
	return it.IsConditionTrue(v1.IntegrationConditionImporting) && !it.IsConditionTrue(v1.IntegrationConditionImported)
}
