...
func (t *deployerTrait) Reverse(e *Environment, traits *v1.Traits) error {
	...
	e.PostActions = append(e.PostActions, func(env *Environment) error {
		for _, resource := range env.Resources.Items() {
			// We need to delete as certain resources are immutable by design in Kubernetes
			// https://github.com/kubernetes/kubernetes/issues/50808
			// For example, Deployment selectors are immutable and Camel K needs its default
			// selectors to work properly. The strategy is to delete and let the operator
			// to create any new from scratch and manage from there.
			err := env.Client.Delete(env.Ctx, resource, &ctrl.DeleteOptions{})
			if err != nil {
				return err
			}
		}
