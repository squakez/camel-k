...
func (t *deployerTrait) Reverse(e *Environment, traits *v1.Traits) error {
	// Register a post action that update the resources generated by the traits (it will transfer ownership to the operator)
	e.PostActions = append(e.PostActions, func(env *Environment) error {
		for _, resource := range env.Resources.Items() {
			// TODO we should look for and upgrade any other manager found
			CSAManagers := []string{"kubectl-client-side-apply"}
			patch, err := csaupgrade.UpgradeManagedFieldsPatch(
				resource,
				k8ssets.New(CSAManagers...),
				"camel-k-operator",
			)
			if err != nil {
				return err
			...
			p := ctrl.RawPatch(types.JSONPatchType, patch)
			err = env.Client.Patch(env.Ctx, resource, p, ctrl.FieldOwner("camel-k-operator"))
			if err != nil {
				return err
			}

			//err := env.Client.Patch(env.Ctx, resource, &upgradeManagedFieldPatch{}, ctrl.FieldOwner("camel-k-operator"))
			// err := env.Client.Patch(env.Ctx, resource, &managedFieldPatch{}, ctrl.FieldOwner("camel-k-operator"))
			// if err != nil {
			// 	return err
			// }
		}
...
type managedFieldPatch struct {
}

func (m *managedFieldPatch) Type() types.PatchType {
	return types.JSONPatchType
}

func (m *managedFieldPatch) Data(obj ctrl.Object) ([]byte, error) {
	return []byte(`[{"op": "replace", "path": "/metadata/managedFields", "value": [{}]}, {"op": "remove", "path": "/metadata/annotations/kubectl.kubernetes.io~1last-applied-configuration"}]`), nil
}

type upgradeManagedFieldPatch struct {
}

func (m *upgradeManagedFieldPatch) Type() types.PatchType {
	return types.JSONPatchType
}

func (m *upgradeManagedFieldPatch) Data(obj ctrl.Object) ([]byte, error) {
	CSAManagers := []string{"kubectl-client-side-apply"}
	patch, err := csaupgrade.UpgradeManagedFieldsPatch(
		obj,
		k8ssets.New(CSAManagers...),
	...
	if err != nil {
		fmt.Println("********* Error creating patch for resource", obj.GetName())
		return nil, err
	}
	...
	fmt.Println("****** Upgrading patch", string(patch))
	return patch, err
}
