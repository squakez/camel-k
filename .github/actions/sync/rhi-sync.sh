...
-i, --interactive             Enable cherry pick interactive mode
...
METADATA_REPO=""
METADATA_BRANCH=""
METADATA_MANIFEST_DIR="distgit/containers/fuse-camel-k-prod-operator-metadata/manifests"
UPSTREAM_ORG=""
UPSTREAM_REPO=""
UPSTREAM_BRANCH=""
...
main() {
  ...
  #TODO: set identity properly as part of the action
  ...
  then
    ...
    cd ..
    clone_gitlab $METADATA_ORG $METADATA_REPO
    cd $METADATA_REPO
    git checkout origin/$METADATA_BRANCH
    cp ../$UPSTREAM_REPO$UPSTREAM_MANIFEST_DIR/*.yaml $METADATA_MANIFEST_DIR/.
    git add $METADATA_MANIFEST_DIR/.
    git commit -m "Manifests directory refresh"
    # We must verify that there is some change to be applied and create a merge request
    if [[ $? == 0 ]] && [[ "$DRY_RUN" == "false" ]]
    then
      # We need to push to a new branch and create a merge request from there
      # This is needed to perform the proper synchronization with dist-git
      BRANCHNAME="chore/regen_$(date '+%Y%m%d')"
      echo "📌 pushing to metadata repo (new branch $BRANCHNAME)"
      git push origin HEAD:refs/heads/$BRANCHNAME
      # Let's make sure that the user is creating the merge request (manually)
      echo "🖐  PLEASE, remind to create a manual merge request from $BRANCHNAME to $METADATA_BRANCH"
      # default for non-interactive, quit
      k="q"
      if [[ "$INTERACTIVE" == "true" ]] ; then
        echo "I can wait while you create a merge request, no rush for me."
        echo "What do you want to do? c) merge request created, you can continue q) quit process"
        read -n 1 k <&1
      fi
      if [[ "$k" == "q" ]] ; then
        # quit
      ...
      echo "❗ dry-run mode on or no changes detected, won't push any change!"
    fi
  fi
}
...
clone_gitlab(){
  ...
  git clone git@gitlab.cee.redhat.com:$org/${repo}.git
}
