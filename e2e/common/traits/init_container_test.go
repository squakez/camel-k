...
func TestInitContainerTrait(t *testing.T) {
	...
	WithNewTestNamespace(t, func(ctx context.Context, g *WithT, ns string) {
		t.Run("Init container write a file", func(t *testing.T) {
			name := RandomizedSuffixName("ic")
			g.Expect(KamelRun(t, ctx, ns, "files/init-container.yaml", "-t", "container.image-pull-policy=Always", "-t", "container.request-cpu=0.005", "-t", "container.request-memory=100Mi", "-t", "container.limit-cpu=200m", "-t", "container.limit-memory=500Mi", "--name", name).Execute()).To(Succeed())
