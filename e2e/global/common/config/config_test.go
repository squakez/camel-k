...
func TestRunConfigExamples(t *testing.T) {
	WithNewTestNamespace(t, func(ns string) {
		...
		t.Run("Simple property", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "property-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "property-route"), TestTimeoutMedium).Should(ContainSubstring("test-property"))
		...
		t.Run("Property file", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "property-file-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "property-file-route"), TestTimeoutMedium).Should(ContainSubstring("hello world"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Property precedence", func(t *testing.T) {
			...
			Eventually(IntegrationPodPhase(ns, "property-file-route"), TestTimeoutMedium).Should(Equal(corev1.PodRunning))
			Eventually(IntegrationConditionStatus(ns, "property-file-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "property-file-route"), TestTimeoutMedium).Should(ContainSubstring("hello universe"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Property from ConfigMap", func(t *testing.T) {
			...
			Eventually(IntegrationPodPhase(ns, "property-route"), TestTimeoutMedium).Should(Equal(corev1.PodRunning))
			Eventually(IntegrationConditionStatus(ns, "property-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "property-route"), TestTimeoutMedium).Should(ContainSubstring("my-configmap-property-value"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Property from ConfigMap as property file", func(t *testing.T) {
			...
			Eventually(IntegrationPodPhase(ns, "property-route"), TestTimeoutMedium).Should(Equal(corev1.PodRunning))
			Eventually(IntegrationConditionStatus(ns, "property-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "property-route"), TestTimeoutMedium).Should(ContainSubstring("my-configmap-property-entry"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Property from Secret", func(t *testing.T) {
			...
			Eventually(IntegrationPodPhase(ns, "property-route"), TestTimeoutMedium).Should(Equal(corev1.PodRunning))
			Eventually(IntegrationConditionStatus(ns, "property-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "property-route"), TestTimeoutMedium).Should(ContainSubstring("my-secret-property-value"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Property from Secret as property file", func(t *testing.T) {
			...
			Eventually(IntegrationPodPhase(ns, "property-route"), TestTimeoutMedium).Should(Equal(corev1.PodRunning))
			Eventually(IntegrationConditionStatus(ns, "property-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "property-route"), TestTimeoutMedium).Should(ContainSubstring("my-secret-property-entry"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Config configmap", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "config-configmap-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "config-configmap-route"), TestTimeoutMedium).Should(ContainSubstring(cmData["my-configmap-key"]))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Resource configmap", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "resource-configmap-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "resource-configmap-route"), TestTimeoutMedium).Should(ContainSubstring(cmData["my-configmap-key"]))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Resource configmap with destination", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "resource-configmap-location-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "resource-configmap-location-route"), TestTimeoutMedium).Should(ContainSubstring(cmData["my-configmap-key"]))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Resource configmap with filtered key and destination", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "resource-configmap-key-location-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "resource-configmap-key-location-route"), TestTimeoutMedium).ShouldNot(ContainSubstring(cmDataMulti["my-configmap-key"]))
			Eventually(IntegrationLogs(ns, "resource-configmap-key-location-route"), TestTimeoutMedium).Should(ContainSubstring(cmDataMulti["my-configmap-key-2"]))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Config configmap as property file", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "config-configmap-properties-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "config-configmap-properties-route"), TestTimeoutMedium).Should(ContainSubstring("hello world"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Config secret", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "config-secret-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "config-secret-route"), TestTimeoutMedium).Should(ContainSubstring(secData["my-secret-key"]))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Resource secret", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "resource-secret-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "resource-secret-route"), TestTimeoutMedium).Should(ContainSubstring(secData["my-secret-key"]))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Plain text configuration file", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "config-file-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "config-file-route"), TestTimeoutMedium).Should(ContainSubstring("the file body"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
			Eventually(AutogeneratedConfigmapsCount(ns), TestTimeoutMedium).Should(Equal(0))
		})
		...
		t.Run("Secret with filtered key", func(t *testing.T) {
			...
			Eventually(IntegrationPodPhase(ns, "config-secret-key-route"), TestTimeoutMedium).Should(Equal(corev1.PodRunning))
			Eventually(IntegrationConditionStatus(ns, "config-secret-key-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "config-secret-key-route"), TestTimeoutMedium).ShouldNot(ContainSubstring(secDataMulti["my-secret-key"]))
			Eventually(IntegrationLogs(ns, "config-secret-key-route"), TestTimeoutMedium).Should(ContainSubstring(secDataMulti["my-secret-key-2"]))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Plain text resource file", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "resource-file-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "resource-file-route"), TestTimeoutMedium).Should(ContainSubstring("the file body"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
			Eventually(AutogeneratedConfigmapsCount(ns), TestTimeoutMedium).Should(Equal(0))
		})
		...
		t.Run("Plain text resource file with destination path", func(t *testing.T) {
			...
			Eventually(IntegrationLogs(ns, "resource-file-location-route"), TestTimeoutMedium).Should(ContainSubstring("the file body"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
			Eventually(AutogeneratedConfigmapsCount(ns), TestTimeoutMedium).Should(Equal(0))
		})
		...
		t.Run("Binary (zip) resource file", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "resource-file-binary-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "resource-file-binary-route"), TestTimeoutMedium).Should(ContainSubstring("the file body"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
			Eventually(AutogeneratedConfigmapsCount(ns), TestTimeoutMedium).Should(Equal(0))
		})
		...
		t.Run("Base64 compressed binary resource file", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "resource-file-base64-encoded-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "resource-file-base64-encoded-route"), TestTimeoutMedium).Should(ContainSubstring(string(expectedBytes)))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
			Eventually(AutogeneratedConfigmapsCount(ns), TestTimeoutMedium).Should(Equal(0))
		})
		...
		t.Run("Build time property", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "build-property-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "build-property-route"), TestTimeoutMedium).Should(ContainSubstring("my-super-application"))
		...
		t.Run("Build time property updated", func(t *testing.T) {
			...
			Eventually(IntegrationConditionStatus(ns, "build-property-route-updated", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "build-property-route-updated"), TestTimeoutMedium).Should(ContainSubstring("my-super-application-updated"))
		...
		t.Run("Build time property file", func(t *testing.T) {
			...
			Eventually(IntegrationLogs(ns, "build-property-file-route"), TestTimeoutMedium).Should(ContainSubstring("my-super-application"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Build time property from ConfigMap", func(t *testing.T) {
			...
			Eventually(IntegrationPodPhase(ns, "build-property-file-route"), TestTimeoutMedium).Should(Equal(corev1.PodRunning))
			Eventually(IntegrationConditionStatus(ns, "build-property-file-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "build-property-file-route"), TestTimeoutMedium).Should(ContainSubstring("my-cool-application"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Build time property from ConfigMap as property file", func(t *testing.T) {
			...
			Eventually(IntegrationPodPhase(ns, "build-property-file-route"), TestTimeoutMedium).Should(Equal(corev1.PodRunning))
			Eventually(IntegrationConditionStatus(ns, "build-property-file-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "build-property-file-route"), TestTimeoutMedium).Should(ContainSubstring("my-super-cool-application"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Build time property from Secret", func(t *testing.T) {
			...
			Eventually(IntegrationPodPhase(ns, "build-property-file-route"), TestTimeoutMedium).Should(Equal(corev1.PodRunning))
			Eventually(IntegrationConditionStatus(ns, "build-property-file-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "build-property-file-route"), TestTimeoutMedium).Should(ContainSubstring("my-great-application"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
		...
		t.Run("Build time property from Secret as property file", func(t *testing.T) {
			...
			Eventually(IntegrationPodPhase(ns, "build-property-file-route"), TestTimeoutMedium).Should(Equal(corev1.PodRunning))
			Eventually(IntegrationConditionStatus(ns, "build-property-file-route", v1.IntegrationConditionReady), TestTimeoutMedium).Should(Equal(corev1.ConditionTrue))
			Eventually(IntegrationLogs(ns, "build-property-file-route"), TestTimeoutMedium).Should(ContainSubstring("my-awsome-application"))
			Expect(Kamel("delete", "--all", "-n", ns).Execute()).To(Succeed())
		})
	})
}
